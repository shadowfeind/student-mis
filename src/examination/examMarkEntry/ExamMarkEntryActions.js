...
export const getEventScheduleAction =
  (year, program, classId, section, event) => async (dispatch) => {
    try {
      ...
      const { data } =
        await axios.get(`${API_URL}/api/GetActiveExamScheduleListForExamMarkEntry/${year}/${program}/${classId}/${section}/${event}/2
    `,tokenConfig);
...
export const getExamEntrySearchDataAction =
  ...
  async (dispatch) => {
    try {
      ...
      const { data } =
        await axios.get(`${API_URL}/api/GetLstAcademicStudentExamData/${year}/${program}/${classId}/${section}/${shift}/${event}/${schedule}/1
    `,tokenConfig);
...
export const getExamEntryBulkAction =
  ...
  async (dispatch) => {
    try {
      ...
      const { data } =
        await axios.get(`${API_URL}/api/GetBulk/${year}/${program}/${classId}/${section}/${shift}/${event}/${schedule}/1
    `,tokenConfig);
